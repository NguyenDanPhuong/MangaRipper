...
namespace MangaRipper.Core
{
    public class MangaFoxImpl : IManga
    {
        ...
        public async Task<IList<Chapter>> FindChapters(string mangaLink)
        {
            var downloader = new Downloader();
            var parser = new Parser();

            // find all chapters in a manga
            string input = await downloader.DownloadStringAsync(mangaLink);
        ...
        public async Task<IList<string>> FindImanges(Chapter chapter)
        {
            ...
            var imgs = parser.Parse("<img src=\"(?<Value>[^\"]+)\"[ ]+onerror", pageData, "Value");
